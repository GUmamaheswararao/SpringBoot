		...
		try {
		    ...
		    if (skillSet == null) {
		      this.findAll().forEach(jobSeekerList::add);
		    	...
		    	Query query = entityManager.createNativeQuery("SELECT js.* FROM job_seeker as js " +
		        ...
		        jobSeekerList.stream().forEach(s -> { 
					s.setPercent(calculatePercentage(skillSet, s));
		        }); 
		        Collections.sort(jobSeekerList, new Comparator<JobSeeker>() {
		            @Override
		            public int compare(JobSeeker c1, JobSeeker c2) {
		                return Double.compare(c2.getPercent(), c1.getPercent());
		            }
		        });
		        //Collections.reverse(jobSeekerList);
